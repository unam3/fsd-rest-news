...
processArgs _ =
  Left
    "Exactly 6 arguments needed: port to run rest-news, db hostname, db port, db user, db password, db name"
  ...
  L.withLogger
    ...
    (\loggerH ->
       case processArgs args of
         Left error' -> L.hError loggerH error' >> exitFailure
