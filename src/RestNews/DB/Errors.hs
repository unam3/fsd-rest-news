...
instance ToJSON MustNotBeNegative where
  toJSON (MustNotBeNegative error') = object ["name" .= error']
...
instance ToJSON DoesNotExist where
  toJSON (DoesNotExist error') = object ["name" .= error']

makeDoesNotExist :: Text -> DoesNotExist
makeDoesNotExist thing = DoesNotExist $ append thing " does not exist"
...
instance ToJSON AlreadyExist where
  toJSON (AlreadyExist error') = object ["name" .= error']

makeAlreadyExist :: Text -> AlreadyExist
makeAlreadyExist thing = AlreadyExist $ append thing " already exist"
...
instance ToJSON UserAlreadyAuthor where
  toJSON (UserAlreadyAuthor error') = object ["name" .= error']
...
instance ToJSON CategoryInUse where
  toJSON (CategoryInUse error') = object ["name" .= error']
...
instance ToJSON NoSuchThing where
  toJSON (NoSuchThing error') = object ["name" .= error']

makeNoSuchThing :: Text -> NoSuchThing
makeNoSuchThing thing = NoSuchThing $ append "no such " thing
...
newtype TagReferencedByArticle =
  TagReferencedByArticle
    { error :: Text
    }
  deriving (Show)

instance ToJSON TagReferencedByArticle where
  toJSON (TagReferencedByArticle error') = object ["name" .= error']

eTagReferencedByArticle :: TagReferencedByArticle
eTagReferencedByArticle =
  TagReferencedByArticle "tag is referenced by an article"
...
instance ToJSON WrongUsernameOrPassword where
  toJSON (WrongUsernameOrPassword error') = object ["name" .= error']
