...
ePSQL_STRING_DATA_RIGHT_TRUNCATION =
  Left
    (QueryError
       ...
       , "9"
       , "2"
       , "\"PATCHEDttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttt\""
       , "\"PATCHED\""
       , "\"fs\""
       , "[\"9\", \"2\"]"
       , "[1, 2]"
    ...
    (QueryError
       ...
       (ResultError
          (ServerError
             ...
             (Just
                "Key (category_id)=(1) is still referenced from table \"categories\".")
             Nothing)))
...
eUnexpectedAmountOfRows0 =
  Left
    (QueryError
       ...
       ["\"usasdername5\"", "\"12345\""]
       (ResultError (UnexpectedAmountOfRows 0)))
...
spec =
  describe "getError" $ do
    ...
    it "process psql_invalid_row_count_in_result_offset_clause" $
      shouldBe (getError ePSQL_INVALID_ROW_COUNT_IN_RESULT_OFFSET_CLAUSE) $
      Just
